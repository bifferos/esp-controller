...
type Config struct {
	...
	VariableFilesDir  string   `json:"variableFilesDir"`
	TokenFilesDir     string   `json:"tokenFilesDir"`
...
func main() {
	...
	cfg := Config{
		VariableList:      []string{"example-device1", "example-device2"},
		TemplatesDir:      "/usr/share/esp-controller-ui/templates",
		StaticDir:         "/usr/share/esp-controller-ui/static",
		VariableFilesDir:  "/var/lib/esp-controller/state",
		TokenFilesDir:     "/var/lib/esp-controller/tokens",
		ExternalIpAddress: externalIp,
		Port:              6060,
	}
	configPath := flag.String("config", "/etc/esp-controller/config.json", "Path to config file")
